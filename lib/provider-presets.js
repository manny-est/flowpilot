...
//                       confirmed against.
//   deploymentClass   — "cloud" (a managed frontier API) or "local" (a
//                       self-hosted server) — ADR-013's local-vs-cloud
//                       signal for auto capability routing. `null` for
//                       "custom", since that preset is deliberately
//                       ambiguous (ADR-012: "the deliberate escape hatch
//                       for any provider/setup a named preset doesn't
//                       cover") — a `null` here means "ask the provider's
//                       own baseUrl instead" (see `isLocalDeployment`,
//                       flowpilot.js), not "treat as cloud."
