const http = require("http");
const https = require("https");
const { isModelsListShaped } = require("./provider-shape-check");

// ---- Base URL normalization (0.6.3, §2A; extended pre-beta.1) -------------
// FlowPilot used to unconditionally append "/v1/chat/completions" (or
// "/v1/models") to whatever's stored as baseUrl, after only trimming a
// trailing slash. Every provider this adapter targets (confirmed against
// each one's own current official docs, 2026-09-23 — OpenAI, OpenRouter,
// Ollama, LM Studio, LocalAI, llama.cpp server, vLLM) documents its base URL
// as ALREADY including "/v1" — so a user who follows the docs and pastes
// e.g. OpenRouter's own "https://openrouter.ai/api/v1" got a doubled
// ".../api/v1/v1/chat/completions" 404. This is a pure function applied at
// REQUEST-CONSTRUCTION TIME ONLY — the stored baseUrl itself is never
// rewritten, so ADR-007's confirmedBaseUrl binding (keyed on the exact
// stored string) stays valid for every existing confirmed provider with no
// forced re-confirmation on upgrade.
//
// ADR-012's three resolution rules, checked in order by chatCompletionsUrl/
// modelsUrl below (recorded in ADR-012 itself, not just here):
//   (a) baseUrl already ends in "/chat/completions" -> used VERBATIM, no
//       "/v1" ever inserted. This is what lets a non-"/v1" gateway path
//       (e.g. Open WebUI's own "/api/chat/completions") work through the
//       Custom preset — the OLD logic stripped this suffix unconditionally
//       and then forced a "/v1" into a path that was never supposed to
//       have one, breaking it. The sibling "/models" is derived by
//       swapping the tail — best-effort, since not every gateway exposes
//       one at that path (listModels() already degrades gracefully on any
//       failure, per its own doc comment).
//   (b) baseUrl ends in "/v1" (with anything or nothing before it, e.g.
//       ".../api/v1") -> append only "/chat/completions"/"/models", never
//       a second "/v1". Already the existing normalizeApiBase behavior
//       below — no new code needed for this rule specifically.
//   (c) Bare host, or any other path -> unchanged pre-existing behavior:
//       ensure a trailing "/v1" (recovering from a pasted "/chat/
//       completions" or "/models" suffix first, so pasting either kind of
//       full endpoint is still idempotent), then append. This is what
//       keeps every existing reverse-proxy/plain-host setup working
//       exactly as before on upgrade.
//
// Recovers a clean ".../v1" (or ".../api/v1", etc. — whatever prefix the
// user's own value already carries) base from any of: a bare host
// ("https://host[:port]"), a host already ending in "/v1" or deeper
// ("https://host/api/v1"), or a full endpoint the user pasted verbatim
// ("https://host/v1/chat/completions", ".../v1/models") — the last two
// are stripped back to their base first, so the result is idempotent
// whether the user pasted a base URL or a full endpoint URL. Trailing
// slashes are tolerated anywhere in the input. Only reached for rule
// (b)/(c) inputs — rule (a) is handled directly by chatCompletionsUrl/
// modelsUrl below, before this function is ever called.
function normalizeApiBase(rawBaseUrl) {
  let base = String(rawBaseUrl || "").trim().replace(/\/+$/, "");
  // A pasted full endpoint (chat completions or models) recovers to its
  // own base — strip it before deciding whether "/v1" is already present.
  base = base.replace(/\/(chat\/completions|models)$/, "").replace(/\/+$/, "");
  if (!/\/v1$/.test(base)) {
    base = base + "/v1";
  }
  return base;
}

// Rule (a) check, shared by chatCompletionsUrl/modelsUrl: does the raw,
// trimmed baseUrl already name a full chat/completions endpoint? If so it
// wins verbatim over everything below — see the block comment above.
function endsWithChatCompletions(rawBaseUrl) {
  return /\/chat\/completions$/.test(String(rawBaseUrl || "").trim().replace(/\/+$/, ""));
}

function chatCompletionsUrl(rawBaseUrl) {
  const trimmed = String(rawBaseUrl || "").trim().replace(/\/+$/, "");
  if (endsWithChatCompletions(trimmed)) {
    return trimmed;
  }
  return normalizeApiBase(rawBaseUrl) + "/chat/completions";
}

function modelsUrl(rawBaseUrl) {
  const trimmed = String(rawBaseUrl || "").trim().replace(/\/+$/, "");
  if (endsWithChatCompletions(trimmed)) {
    return trimmed.replace(/\/chat\/completions$/, "/models");
  }
  return normalizeApiBase(rawBaseUrl) + "/models";
}

// ---- OpenRouter-specific request extras (0.6.3, §2D) -----------------------
// Both off by default; both are OpenRouter-preset-only — a no-op for every
// other preset, keyed on presetId rather than baseUrl pattern-matching so a
// user who's pointed the OpenRouter preset at a proxy/mirror still gets the
// behavior they asked for.
//   - ZDR: `provider:{zdr:true}` in the request body — confirmed from
//     OpenRouter's own docs. This can only ENFORCE zero-data-retention
//     routing per request; it cannot disable an account-level ZDR
//     enforcement setting if one is already active.
//   - Attribution: `HTTP-Referer` + `X-Title` headers — optional per
//     OpenRouter's own docs, used for their public rankings/analytics.
//     App identity, not user data (§5 open decision #1) — off until Manny
//     decides; the toggle exists either way so it's a settings change, not
//     a code change, once decided.
const FLOWPILOT_REPO_URL = "https://github.com/manny-est/flowpilot";

function applyOpenRouterExtras(settings, headers, body) {
  if (!settings || settings.presetId !== "openrouter") { return; }
  if (settings.zdrEnabled) {
    body.provider = Object.assign({}, body.provider, { zdr: true });
  }
  if (settings.attributionEnabled) {
    headers["HTTP-Referer"] = FLOWPILOT_REPO_URL;
    headers["X-Title"] = "FlowPilot";
  }
}

function postJson(urlString, headers, body, timeoutMs) {
  return new Promise((resolve, reject) => {
    let url;
    try {
      url = new URL(urlString);
    } catch (err) {
      reject(new Error(`Invalid provider URL: ${urlString}`));
      return;
    }

    const payload = JSON.stringify(body);
    const transport = url.protocol === "https:" ? https : http;
    const options = {
      method: "POST",
      hostname: url.hostname,
      port: url.port || (url.protocol === "https:" ? 443 : 80),
      path: `${url.pathname}${url.search}`,
      headers: Object.assign({
        "Content-Type": "application/json",
        "Content-Length": Buffer.byteLength(payload)
      }, headers || {})
    };

    const req = transport.request(options, (res) => {
      let data = "";
      res.setEncoding("utf8");
      res.on("data", chunk => { data += chunk; });
      res.on("end", () => {
        let parsed = null;
        try {
          parsed = data ? JSON.parse(data) : null;
        } catch (err) {
          // Never echo the raw upstream body — a security boundary, not just
          // tidiness. This request may be the provider-confirmation check
          // hitting a baseUrl for the first time (SSRF mitigation, ADR-007);
          // an attacker-controlled target (internal service, cloud metadata)
          // must not be able to get its response body reflected back to the
          // caller through a FlowPilot error message.
          reject(new Error(`Provider returned a non-JSON response (status ${res.statusCode}).`));
          return;
        }

        if (res.statusCode < 200 || res.statusCode >= 300) {
          const msg = parsed && parsed.error ? JSON.stringify(parsed.error) : `status ${res.statusCode}`;
          reject(new Error(`Provider request failed (${res.statusCode}): ${msg}`));
          return;
        }

        resolve(parsed);
      });
    });

    req.on("error", reject);
    const effectiveTimeoutMs = timeoutMs || 180000;
    req.setTimeout(effectiveTimeoutMs, () => {
      req.destroy(new Error(`Provider request timed out after ${effectiveTimeoutMs}ms — increase the request timeout in Settings → Behavior for slower hardware.`));
    });

    req.write(payload);
    req.end();
  });
}

function getJson(urlString, headers, timeoutMs) {
  return new Promise((resolve, reject) => {
    let url;
    try {
      url = new URL(urlString);
    } catch (err) {
      reject(new Error(`Invalid provider URL: ${urlString}`));
      return;
    }

    const transport = url.protocol === "https:" ? https : http;
    const options = {
      method: "GET",
      hostname: url.hostname,
      port: url.port || (url.protocol === "https:" ? 443 : 80),
      path: `${url.pathname}${url.search}`,
      headers: headers || {}
    };

    const req = transport.request(options, (res) => {
      let data = "";
      res.setEncoding("utf8");
      res.on("data", chunk => { data += chunk; });
      res.on("end", () => {
        let parsed = null;
        try {
          parsed = data ? JSON.parse(data) : null;
        } catch (err) {
          // See postJson above — never echo the raw upstream body.
          reject(new Error(`Provider returned a non-JSON response (status ${res.statusCode}).`));
          return;
        }

        if (res.statusCode < 200 || res.statusCode >= 300) {
          const msg = parsed && parsed.error ? JSON.stringify(parsed.error) : `status ${res.statusCode}`;
          reject(new Error(`Provider request failed (${res.statusCode}): ${msg}`));
          return;
        }

        resolve(parsed);
      });
    });

    req.on("error", reject);
    const effectiveTimeoutMs = timeoutMs || 30000;
    req.setTimeout(effectiveTimeoutMs, () => {
      req.destroy(new Error(`Provider request timed out after ${effectiveTimeoutMs}ms — increase the request timeout in Settings → Behavior for slower hardware.`));
    });

    req.end();
  });
}

// ---------------------------------------------------------------------
// Models dropdown: lists models via the OpenAI-compatible GET /v1/models
// endpoint, so the settings UI can suggest valid model names instead of
// the user guessing and hitting a 404 from /v1/chat/completions. Unlike
// chat()/probeTools(), settings.model is NOT required here — this is how
// the user picks one. Never throws: a provider without /v1/models (or any
// other failure) just means an empty list with an explanatory error, which
// the UI shows as a hint while leaving the model field free-text.
//
// Allowed to run against an UNCONFIRMED provider (ADR-007) — same blind
// treatment as /flowpilot/test and /flowpilot/probe: getJson's error path
// is already generic (never reflects the upstream body), and a "successful"
// response is only trusted if it's genuinely models-list-shaped
// (isModelsListShaped) — a non-provider target returning some unrelated
// JSON blob with data[].id-shaped entries doesn't get those ids reflected
// back to the client. This route never writes confirmedBaseUrl/confirmedAt;
// only /flowpilot/test performs the deliberate confirming action.
// ---------------------------------------------------------------------
async function listModels(settings) {
  if (!String(settings.baseUrl || "").trim()) throw new Error("Provider base URL is required.");

  const headers = {};
  if (settings.apiKey) {
    headers.Authorization = `Bearer ${settings.apiKey}`;
  }

  try {
    const response = await getJson(modelsUrl(settings.baseUrl), headers, settings.requestTimeoutMs || 30000);
    if (!isModelsListShaped(response)) {
      return { models: [], error: "Not a valid provider endpoint (no FlowPilot-compatible response)." };
    }
    const models = response.data
      .map(function (m) { return m && m.id; })
      .filter(function (id) { return typeof id === "string" && id; });
    return { models: models };
  } catch (err) {
    return { models: [], error: err.message };
  }
}

// `options.tools` (OpenAI-style tool/function definitions) and
// `options.toolChoice` are optional — when present, the request asks the
// provider to call a tool instead of (or in addition to) replying with text.
// `result.toolCalls` is the raw `message.tool_calls` array (or null), passed
// through unparsed so the agent loop can validate/dispatch each call itself.
// A tool-call-only response has no `content`, so the "[No assistant message
// returned...]" fallback only applies when there are no tool calls either.
async function chat(settings, messages, options) {
  if (!String(settings.baseUrl || "").trim()) throw new Error("Provider base URL is required.");
  if (!settings.model) throw new Error("Model is required.");

  const headers = {};
  if (settings.apiKey) {
    headers.Authorization = `Bearer ${settings.apiKey}`;
  }

  const temperature = settings.temperature !== undefined ? Number(settings.temperature) : 0.2;

  const body = {
    model: settings.model,
    messages,
    temperature,
    stream: false
  };
  if (options && Array.isArray(options.tools) && options.tools.length) {
    body.tools = options.tools;
    body.tool_choice = options.toolChoice || "auto";
  }
  if (options && options.responseFormat) {
    body.response_format = options.responseFormat;
  }
  if (options && Number.isInteger(options.maxTokens) && options.maxTokens > 0) {
    body.max_tokens = options.maxTokens;
  }
  applyOpenRouterExtras(settings, headers, body);

  const startedAt = Date.now();
  const response = await postJson(chatCompletionsUrl(settings.baseUrl), headers, body, settings.requestTimeoutMs || 180000);
  const totalMs = Date.now() - startedAt;

  const message = response && response.choices && response.choices[0] && response.choices[0].message;
  const content = message ? message.content : "";
  const toolCalls = message && Array.isArray(message.tool_calls) && message.tool_calls.length
    ? message.tool_calls
    : null;

  return {
    raw: response,
    content: content || (toolCalls ? "" : "[No assistant message returned by provider]"),
    toolCalls: toolCalls,
    finishReason: (response && response.choices && response.choices[0] &&
      response.choices[0].finish_reason) || null,
    timing: { totalMs },
    usage: (response && response.usage) || null
  };
}

// Streaming variant of chat(): requests `stream: true` and parses the
// OpenAI-compatible SSE format (lines `data: {...}`, terminated by
// `data: [DONE]`). Calls onDelta(text) for each content fragment as it
// arrives and resolves with { content } containing the full concatenated
// text once the stream ends. Used for chat streaming only.
// Splits a streaming content string on <think>...</think> tags so that
// reasoning text is routed to onReasoning and the actual response text to
// onContent. Handles tags that arrive split across multiple chunks via a
// small lookahead buffer. Handles both:
//   - SGLang/Nemotron style: delta.reasoning_content (separate field)
//   - llama.cpp/LocalAI style: <think>...</think> embedded in delta.content
// The two paths converge at onReasoningDelta in postStream — callers see
// one uniform reasoning callback regardless of provider format.
function createThinkTagSplitter(onContent, onReasoning) {
  let phase = "seek"; // "seek" | "think" | "content"
  let buf = "";
  const OPEN = "<think>";
  const CLOSE = "</think>";

  function push(text) {
    buf += text;
    while (buf.length > 0) {
      if (phase === "seek") {
        const idx = buf.indexOf(OPEN);
        if (idx === -1) {
          // No opening tag visible — emit everything except the last few bytes
          // that might be a partial tag, buffer the rest.
          const safe = buf.length > OPEN.length - 1 ? buf.length - (OPEN.length - 1) : 0;
          if (safe > 0) { onContent(buf.slice(0, safe)); buf = buf.slice(safe); }
          break;
        }
        if (idx > 0) { onContent(buf.slice(0, idx)); }
        buf = buf.slice(idx + OPEN.length);
        phase = "think";
      } else if (phase === "think") {
        const idx = buf.indexOf(CLOSE);
        if (idx === -1) {
          const safe = buf.length > CLOSE.length - 1 ? buf.length - (CLOSE.length - 1) : 0;
          if (safe > 0) { onReasoning(buf.slice(0, safe)); buf = buf.slice(safe); }
          break;
        }
        if (idx > 0) { onReasoning(buf.slice(0, idx)); }
        buf = buf.slice(idx + CLOSE.length);
        // Skip whitespace/newline immediately after </think>
        const trimmed = buf.replace(/^\s+/, "");
        buf = trimmed;
        phase = "content";
      } else {
        onContent(buf);
        buf = "";
        break;
      }
    }
  }

  function finish() {
    if (!buf) { return; }
    if (phase === "think") { onReasoning(buf); } else { onContent(buf); }
    buf = "";
  }

  return { push, finish };
}

function postStream(urlString, headers, body, timeoutMs, onDelta, onReasoningDelta) {
  return new Promise((resolve, reject) => {
    let url;
    try {
      url = new URL(urlString);
    } catch (err) {
      reject(new Error(`Invalid provider URL: ${urlString}`));
      return;
    }

    const payload = JSON.stringify(body);
    const transport = url.protocol === "https:" ? https : http;
    const options = {
      method: "POST",
      hostname: url.hostname,
      port: url.port || (url.protocol === "https:" ? 443 : 80),
      path: `${url.pathname}${url.search}`,
      headers: Object.assign({
        "Content-Type": "application/json",
        "Content-Length": Buffer.byteLength(payload)
      }, headers || {})
    };

    const startedAt = Date.now();
    let firstTokenAt = null;
    let usage = null;
    let finishReason = null;

    const req = transport.request(options, (res) => {
      res.setEncoding("utf8");

      if (res.statusCode < 200 || res.statusCode >= 300) {
        // Drain the body but never reflect it — see postJson's comment above.
        res.on("data", () => {});
        res.on("end", () => {
          reject(new Error(`Provider request failed (status ${res.statusCode}).`));
        });
        return;
      }

      let sseBuf = "";
      let sawValidSseData = false;
      let full = "";
      // When onReasoningDelta is provided, intercept <think>...</think> from
      // delta.content in addition to the dedicated delta.reasoning_content field
      // (llama.cpp/LocalAI style vs SGLang/Nemotron style — both converge here).
      const thinkSplitter = onReasoningDelta
        ? createThinkTagSplitter(
            function (c) { if (firstTokenAt === null) { firstTokenAt = Date.now(); } full += c; onDelta(c); },
            onReasoningDelta
          )
        : null;

      res.on("data", (chunk) => {
        sseBuf += chunk;
        const lines = sseBuf.split("\n");
        sseBuf = lines.pop(); // keep the last (possibly partial) line for next time

        lines.forEach((line) => {
          line = line.trim();
          if (!line.startsWith("data:")) { return; }
          const dataStr = line.slice(5).trim();
          if (!dataStr || dataStr === "[DONE]") { return; }

          let evt;
          try {
            evt = JSON.parse(dataStr);
          } catch (e) {
            return; // ignore malformed/partial SSE chunk
          }

          sawValidSseData = true;

          if (evt && evt.usage) { usage = evt.usage; }
          if (evt && evt.choices && evt.choices[0] && evt.choices[0].finish_reason) {
            finishReason = evt.choices[0].finish_reason;
          }

          const deltaObj = evt && evt.choices && evt.choices[0] && evt.choices[0].delta;
          if (deltaObj) {
            if (deltaObj.content) {
              if (thinkSplitter) {
                thinkSplitter.push(deltaObj.content);
              } else {
                if (firstTokenAt === null) { firstTokenAt = Date.now(); }
                full += deltaObj.content;
                onDelta(deltaObj.content);
              }
            }
            // Separate reasoning_content field (SGLang/Nemotron style). Both
            // paths run in parallel — they are mutually exclusive per provider
            // (llama.cpp puts reasoning in delta.content via <think> tags;
            // SGLang puts it in delta.reasoning_content with delta.content "").
            if (deltaObj.reasoning_content && onReasoningDelta) {
              onReasoningDelta(deltaObj.reasoning_content);
            }
          }
        });
      });
      res.on("end", () => {
        if (thinkSplitter) { thinkSplitter.finish(); }
        if (!sawValidSseData) {
          reject(new Error(`Provider returned a non-SSE response (status ${res.statusCode}).`));
          return;
        }
        const endedAt = Date.now();
        resolve({
          content: full,
          ttftMs: firstTokenAt !== null ? firstTokenAt - startedAt : null,
          totalMs: endedAt - startedAt,
          usage: usage,
          finishReason: finishReason
        });
      });
    });

    req.on("error", reject);
    const effectiveTimeoutMs = timeoutMs || 180000;
    req.setTimeout(effectiveTimeoutMs, () => {
      req.destroy(new Error(`Provider request timed out after ${effectiveTimeoutMs}ms — increase the request timeout in Settings → Behavior for slower hardware.`));
    });

    req.write(payload);
    req.end();
  });
}

async function chatStream(settings, messages, onDelta, onReasoningDelta, options) {
  if (!String(settings.baseUrl || "").trim()) throw new Error("Provider base URL is required.");
  if (!settings.model) throw new Error("Model is required.");

  const headers = {};
  if (settings.apiKey) {
    headers.Authorization = `Bearer ${settings.apiKey}`;
  }

  const temperature = settings.temperature !== undefined ? Number(settings.temperature) : 0.2;

  // stream_options.include_usage asks OpenAI-compatible servers to emit a
  // final SSE chunk carrying token usage (no delta) before [DONE]. Providers
  // that don't support it just ignore the option; postStream treats a
  // missing usage field as null either way.
  const body = {
    model: settings.model,
    messages,
    temperature,
    stream: true,
    stream_options: { include_usage: true }
  };
  if (options && options.responseFormat) {
    body.response_format = options.responseFormat;
  }
  if (options && Number.isInteger(options.maxTokens) && options.maxTokens > 0) {
    body.max_tokens = options.maxTokens;
  }
  applyOpenRouterExtras(settings, headers, body);

  const result = await postStream(chatCompletionsUrl(settings.baseUrl), headers,
    body, settings.requestTimeoutMs || 180000, onDelta, onReasoningDelta);

  return {
    content: result.content || "",
    timing: { ttftMs: result.ttftMs, totalMs: result.totalMs },
    usage: result.usage,
    finishReason: result.finishReason
  };
}

// ---------------------------------------------------------------------
// B-cap1: sends a minimal request with a trivial tool definition and a
// prompt that should trigger a tool call, to detect whether this provider
// supports OpenAI-style function/tool calling. Returns
// { supportsTools: boolean }, never throws — a provider that doesn't
// understand "tools" typically 400s on the request itself, which is a clean
// "no" for capability purposes (connectivity is checked separately by
// chat()).
//
// CAUTION: a pass here means the provider is ELIGIBLE for agentic
// tool-calling features, not a reliability guarantee — the agentic path must
// still handle malformed mid-conversation tool calls by falling back to the
// envelope for that turn.
// ---------------------------------------------------------------------
async function probeTools(settings) {
  if (!String(settings.baseUrl || "").trim()) throw new Error("Provider base URL is required.");
  if (!settings.model) throw new Error("Model is required.");

  const headers = {};
  if (settings.apiKey) {
    headers.Authorization = `Bearer ${settings.apiKey}`;
  }

  const tools = [{
    type: "function",
    function: {
      name: "ping",
      description: "Respond to a connectivity probe. Takes no arguments.",
      parameters: { type: "object", properties: {}, additionalProperties: false }
    }
  }];

  const probeBody = {
    model: settings.model,
    messages: [
      { role: "system", content: "You are being tested for tool/function-calling support." },
      { role: "user", content: "Call the \"ping\" tool now with no arguments. Do not reply with text." }
    ],
    tools: tools,
    tool_choice: "auto",
    temperature: 0,
    stream: false
  };
  applyOpenRouterExtras(settings, headers, probeBody);

  try {
    const response = await postJson(chatCompletionsUrl(settings.baseUrl), headers, probeBody, settings.requestTimeoutMs || 30000);

    const message = response && response.choices && response.choices[0] && response.choices[0].message;
    const toolCalls = message && Array.isArray(message.tool_calls) ? message.tool_calls : [];
    const wellFormed = toolCalls.length > 0 && toolCalls.every(function (call) {
      return !!(call && call.function && typeof call.function.name === "string" &&
        typeof call.function.arguments === "string");
    });

    return { supportsTools: wellFormed };
  } catch (err) {
    return { supportsTools: false, error: err.message };
  }
}

// ---------------------------------------------------------------------
// Checks whether the provider's response includes reasoning_content —
// the separate chain-of-thought field emitted by reasoning models
// (e.g. Nemotron, DeepSeek-R1, o1-style). Reuses the connectivity test
// response already obtained by the /test route rather than making a
// second round-trip: caller passes the raw response object.
// Returns { isReasoningModel: boolean }.
// ---------------------------------------------------------------------
function detectReasoning(rawResponse) {
  const message = rawResponse && rawResponse.choices && rawResponse.choices[0]
    && rawResponse.choices[0].message;
  const isReasoningModel = !!(message && message.reasoning_content != null
    && message.reasoning_content !== "");
  return { isReasoningModel };
}

module.exports = { chat, chatStream, probeTools, listModels, detectReasoning, normalizeApiBase, chatCompletionsUrl, modelsUrl };
