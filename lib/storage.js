...
function createStorage(userDir) {
  ...
  // A3: per-conversation transcripts (consolidates the old one-file-per-turn
  // chats/ logs into one JSON Lines file per conversation, keyed by a
  // frontend-generated conversationId). Callers must pass an
  ...
  // D6: removes a conversation's transcript file (e.g. user deletes it from
  // the conversation list). Best-effort — a missing file is not an error.
  ...
  // A3b: Recall searches across OTHER conversations' transcripts. Lists
