...
function createStorage(userDir) {
  ...
  function getSettings() {
    ...
    try {
      ...
      if (Array.isArray(parsed.providers) && parsed.providers.length) {
        merged.providers = parsed.providers.map(normalizePresetId);
      }
  ...
  function saveSettings(settings, options) {
    ...
    if (settings && Array.isArray(settings.providers)) {
      merged.providers = reconcileProviderSecrets(settings.providers, current.providers, !!(options && options.trustConfirmation)).map(normalizePresetId);
    }
