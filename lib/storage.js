...
function createStorage(userDir) {
  ...
  const defaultSettings = {
    ...
    loopHoldStep: false,
    // Opt-in: routes Generate through the Phase 10 step-queue engine (graph
    // read-back verification after import). Off by default — old path stays
    // default until corpus regression testing confirms parity. Only Generate
    // uses this flag in the current build; Modify and Build will follow once
    // Generate is confirmed stable.
    enableStepQueue: false,
