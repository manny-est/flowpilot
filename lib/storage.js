...
function reconcileProviderSecrets(incoming, existing, trustedConfirmation) {
  ...
  return (Array.isArray(incoming) ? incoming : []).map(function (p) {
    ...
    next.routingTier = normalizeRoutingTier(next.routingTier);
...
function createStorage(userDir) {
  ...
  function defaultProvider() {
    return {
      ...
      model: "",
      temperature: 0.2,
      // Manual model-tier routing profile. Absent on older provider
      // profiles; consumers treat missing/invalid values as "A".
      routingTier: "A",
    ...
    try {
      ...
      if (Array.isArray(parsed.providers) && parsed.providers.length) {
        merged.providers = parsed.providers.map(function (p) {
          return normalizePresetId(Object.assign({}, p, { routingTier: normalizeRoutingTier(p && p.routingTier) }));
        });
      }
