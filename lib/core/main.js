    ...
    // Refreshes the "Assistant capability" status line from the active
    // provider's current in-memory state — called after fillProviderFields
    // and again right after Pre-flight check updates supportsTools, so the
    // text is correct whether or not the Settings panel is open at the time.
    function updateTierStatus() {
        el("#fp-tier-status").text(capabilityStatusText(activeProvider()));
    }
