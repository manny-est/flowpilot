    ...
    function collectSettings() {
        ...
        if (!isFinite(agentTurnMaxTokens) || agentTurnMaxTokens < 256) { agentTurnMaxTokens = 4096; }
        agentTurnMaxTokens = Math.round(agentTurnMaxTokens);
    ...
    function executeApplyStepTool(args) {
        args = args || {};
        ...
        if (newNodes.length || newWires.length) {
            idMap = applyInsertions(newNodes, newWires, []) || {};
        ...
        if (changes.length) {
            var propDiffs = changes.filter(function (c) { return c && c.id && c.set && typeof c.set === "object"; })
                .map(function (c) {
                    ...
                    if (hasWireChange && live) {
                        ...
                        var validTargetIds = {};
                        RED.nodes.eachLink(function (l) {
                            if (l.source && l.source.id === c.id && l.target) { validTargetIds[l.target.id] = true; }
                        });
                        wiresDiff = computeWireDiff(c.id, c.set.wires, validTargetIds);
                    }
                    return {
                        modNode: { id: c.id },
            ...
            if (propDiffs.length) { applyModifications(propDiffs, [], null, idMap); }
            changes.forEach(function (c) {
                if (!c || !c.id || !c.set || typeof c.set !== "object") { return; }
                Object.keys(c.set).filter(function (k) { return k !== "wires"; }).forEach(function (k) {
                    // Resolve through idMap the SAME way applyModifications's
                    // Tier 1 just did, so a set value referencing a
                    // placeholder from THIS call's own newNodes (e.g. an
                    // mqtt-out's "broker" pointing at a new mqtt-broker) is
                    // checked against what actually landed, not the raw
                    // unresolved placeholder string.
                    var rawVal = c.set[k];
                    var expected = (idMap && typeof rawVal === "string" && idMap[rawVal]) ? idMap[rawVal] : rawVal;
                    steps.push({ check: "property", nodeId: c.id, prop: k, expected: expected });
                });
                if (Object.prototype.hasOwnProperty.call(c.set, "wires")) {
                    ...
                    desiredWires.forEach(function (targets, port) {
                        (Array.isArray(targets) ? targets : []).forEach(function (targetId) {
                            var toId = (idMap && typeof targetId === "string" && idMap[targetId]) ? idMap[targetId] : targetId;
                            steps.push({ check: "wire", fromId: c.id, fromPort: port, toId: toId });
                        });
                    });
                }
            });
    ...
    function executeRemoveStepTool(args) {
        args = args || {};
        if (!args.nodeId) {
            return buildWriteToolResult(args, [], { error: "remove_step requires nodeId" });
        }
        applyModifications([], [args.nodeId], null, {});
        var checks = runChecksForToolResult([{ check: "absent", nodeId: args.nodeId }], {});
        return buildWriteToolResult(args, checks);
    }

    function executeRenameNodeTool(args) {
        args = args || {};
        if (!args.nodeId || typeof args.name !== "string") {
            return buildWriteToolResult(args, [], { error: "rename_node requires nodeId and name" });
        }
        var live = findLiveNode(args.nodeId);
        var diff = [{
            modNode: { id: args.nodeId },
            propertyChanges: [{ key: "name", oldVal: live ? live.name : undefined, newVal: args.name }],
            wiresChanged: false,
            wiresDiff: { toAdd: [], toRemove: [] }
        }];
        applyModifications(diff, [], null, {});
        var checks = runChecksForToolResult([{ check: "property", nodeId: args.nodeId, prop: "name", expected: args.name }], {});
        return buildWriteToolResult(args, checks);
    }
    ...
    function executeGroupNodesTool(args) {
        args = args || {};
        ...
        var newGroup;
        try {
            newGroup = RED.group.createGroup(resolved);
            if (!newGroup) { throw new Error("createGroup returned nothing"); }
        } catch (e) {
            return buildWriteToolResult(args, [], { error: "Failed to create group: " + ((e && e.message) || e) });
        }
        newGroup.name = name;
    ...
    function executeAgentToolCall(call) {
        ...
        switch (name) {
            ...
            case "apply_step":
                return executeApplyStepTool(args);
            case "remove_step":
                return executeRemoveStepTool(args);
            case "rename_node":
                return executeRenameNodeTool(args);
            case "group_nodes":
                return executeGroupNodesTool(args);
            case "ask_user":
