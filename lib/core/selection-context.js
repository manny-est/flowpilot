// ---------------------------------------------------------------------
// Selection/context module (Phase 9 refactor seam 5) — the group-aware
// selection layer (Phase 8.5 C2) plus the live-canvas context builder
// Generate/Modify/Document/Build/Chat all read from.
//
// Concatenated into the same shared closure as the rest of
// flowpilot-core.js (see lib/build-core-script.js). First seam with real
// RED.* coupling (RED.view.selection, RED.group.getNodes, RED.nodes.node/
// eachLink) — these functions can't be exercised standalone in Node the
// way the pure-function seams could; verification here is syntax +
// line-diff + a live editor smoke test. Also depends on
// `armedExecuteAction`/`pinnedSelectionIds` (declared in main.js — core
// Execute-arming state, a broader concern than selection/context itself,
// so it stayed there) and `sanitizeNode` (lib/core/redaction.js) — both
// reached via the shared closure, not passed in.
// ---------------------------------------------------------------------
