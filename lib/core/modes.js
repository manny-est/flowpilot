    ...
    function runBuildReview(loop) {
        ...
        var instruction = "CONTEXT: You are the fix engine inside a build-test-fix loop. " +
            ...
            "ENOTFOUND, getaddrinfo ENOTFOUND), the flow itself is correctly " +
            "built — the external service is simply not reachable from this " +
            "environment. Reply in plain text acknowledging the flow is " +
            "structurally correct and the network error is an infrastructure " +
            "issue outside the flow. Do NOT propose any changes; this error " +
            "cannot be resolved by modifying the flow. " +
    ...
    function handleBuildReviewResult(data) {
        ...
        if (data.prose) {
            ...
            pushHistory("assistant", explanation);
            renderActionChip(data.suggestedAction);
            var stopMsg = /EHOSTUNREACH|ECONNREFUSED|ETIMEDOUT|ENOTFOUND|getaddrinfo|unreachable|infrastructure/i
                .test(explanation)
