    ...
    function verifyImportedNodes(importResult, todoRec) {
        ...
        var allGood = total > 0 && missing.length === 0;
        if (total === 0) {
            // Nothing verifiable (only comments/groups) — skip notice.
        } else if (allGood) {
            addMessage("fp-notice", "✓ Verified: all " + found + " node(s) confirmed on canvas.");
        } else {
            addMessage("fp-notice", "⚠ Verification: " + found + "/" + total + " node(s) on canvas — " +
                missing.length + " not found after import (" + missing.join(", ") + "). " +
    ...
    function handleBuildResult(data, goalPrompt) {
        ...
        // WS4: surface a consent notice when the proposed flow makes external calls.
        var nodeClasses = data.stepNodeClasses;
        if (nodeClasses && nodeClasses.sideEffecting && nodeClasses.sideEffecting.length > 0) {
            var sideLabels = nodeClasses.sideEffecting.map(function(n) { return n.name || n.type; }).join(", ");
            addMessage("fp-notice", "⚠ External calls: " + sideLabels + " — review before deploying.");
        }
