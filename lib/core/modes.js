    ...
    function runAgentLoop(firstEndpoint, payload, stepExtra, onDone, onError) {
        ...
        function processToolCallsFrom(calls, idx, nextMessages, toolTiers) {
            ...
            var isWriteTool = name === "apply_step" || name === "remove_step" || name === "rename_node";
            ...
            if (name === "ask_user") {
                ...
                renderAskUserQuestion({ question: args.question, options: args.options,
                    onAnswer: function (answerText) { continueWithResult({ answer: answerText }); } });
            ...
            if (writeToolCallNeedsConsent(tier, name, args)) {
                renderAgentToolConsentGate({
                    ...
                    onResume: function (granted) {
                        ...
                        continueWithResult(executeAgentToolCall(call));
                    }
                });
    ...
    function handleModifyResult(data) {
        ...
        var todoRec = null;
        if (planItems.length) {
            // W7: real per-item correlation. Each WRITE tool call this turn
            // resolves ONE plan item, in order (§16 point 2 — "one WRITE
            // tool call = one plan/todo item"), via _agentWriteResults
            // (runAgentLoop, populated only when apply_step/remove_step/
            // rename_node calls actually happened). This is the deferred
            // half of CLAUDE-005 (c6c2e84), now unblocked: that fix could
            // only mark the WHOLE plan active/resolved together because
            // there was no way to know which check proved which numbered
            // line — a per-call tool result finally gives that mapping for
            // free. Items beyond the resolved-call count (or ALL items,
            // when no WRITE tool call was made this turn — the default,
            // non-agentic-write path) fall back to CLAUDE-005's original
            // all-together marking below, resolved via the ordinary
            // aggregate verifySteps envelope path exactly as before —
            // confirmed unchanged when _agentWriteResults is empty/absent.
    ...
    function runSingleVerifyCheck(step, idMap) {
        ...
        case "property": {
            ...
            var okP = !!pNode && pNode[step.prop] === step.expected;
            return { ok: okP, label: (step.prop || "property") + " on " + step.nodeId };
        }
        case "exists": {
            var okE = !!RED.nodes.node(resolve(step.nodeId));
            return { ok: okE, label: step.nodeId + " missing" };
        }
        case "absent": {
            var okA = !RED.nodes.node(resolve(step.nodeId));
            return { ok: okA, label: step.nodeId + " still present" };
        }
        case "wire": {
            var fromNode = RED.nodes.node(resolve(step.fromId));
            var toId = resolve(step.toId);
            var port = step.fromPort || 0;
            var okW = !!(fromNode && Array.isArray(fromNode.wires) &&
                Array.isArray(fromNode.wires[port]) &&
                fromNode.wires[port].indexOf(toId) !== -1);
            return { ok: okW, label: "wire " + step.fromId + " → " + step.toId };
        }
        default:
    ...
    function verifyModifySteps(verifySteps, idMap, todoRec) {
        ...
        verifySteps.forEach(function (step) {
            ...
            if (!result) { return; }
            total++;
            if (result.ok) { passed++; } else { failures.push(result.label); }
        });
    ...
    function runBuildReview(loop) {
        ...
        var instruction = "CONTEXT: You are the fix engine inside a build-test-fix loop. " +
            ...
            "when something needs patching. Do NOT use the <<<FLOWPILOT_DATA>>> " +
            ...
            "genuinely uncertain what to fix, describe the uncertainty inside " +
            "\"explanation\" in a fix envelope.\n\n" +
