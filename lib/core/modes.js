    ...
    function verifyModifySteps(verifySteps, idMap, todoRec) {
        ...
        verifySteps.forEach(function (step) {
            ...
            case "wire": {
                ...
                ok = !!(fromNode && Array.isArray(fromNode.wires) &&
    ...
    // limitation — see the handleBuildResult call site). Modeled on
    // renderLoopCheckpoint's message + fp-chip-row pattern. onDecision(true)
    // = Proceed (today's silent auto-fire behavior), onDecision(false) =
    // Skip (checkpoint auto-verify won't fire for these nodes — see
    // startBuildLoop's skipCheckpoint* resolution and the gates in
    // onDebugMessage/onNodeStatus).
    function renderBuildConsentGate(sideEffecting, onDecision) {
        var $box = el("#fp-messages");
        if (!$box.length) { onDecision(true); return; }

        var labels = sideEffecting.map(function (n) { return n.name || n.type; }).join(", ");
        addMessage("assistant", "This build includes node(s) that make external calls: " + labels +
            ". Proceed to let the deploy-test loop auto-verify them once triggered, or skip " +
            "auto-verify for these (you can still deploy and confirm the result yourself)?");
        ...
        $("<button>")
            .addClass("red-ui-button red-ui-button-small fp-chip fp-question-option")
            .attr("type", "button")
            .text("Skip auto-verify")
            .on("click", function () { decide(false); })
            .appendTo($row);

        $box.append($row);
        addRecord("question", { options: ["Proceed", "Skip auto-verify"], buildConsentGate: true });
        scrollMessagesToBottom();
    }
    ...
    function handleBuildResult(data, goalPrompt) {
        ...
        function showReviewPanel(consentGranted) {
        ...
        if (sideEffecting.length > 0) {
            renderBuildConsentGate(sideEffecting, showReviewPanel);
        } else {
