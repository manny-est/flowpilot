    ...
    function handleModifyResult(data) {
        ...
        // (apply_step/remove_step/rename_node) — each already consent-gated
        // and applied through its own review chip. Hoisted above the
        // planItems.length check (rather than declared inside it) so it's
        // always defined below even on a turn with no Plan: block, e.g. a
        // wrap-up-only final response after tool calls.
