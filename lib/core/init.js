    ...
    var HELP_TEXT = "## Captain's briefing\n\n" +
        ...
        "- `/bug` — report a bug (links to GitHub issues)\n" +
        "- `/feedback` — feature requests / general feedback (GitHub issues, or DM me on the forum)\n" +
    ...
    // /bug: brief info + a straight link to file a GitHub issue.
    var BUG_TEXT = "## Report a bug\n\n" +
        "Something broken? The human crew reads every report.\n\n" +
        "- **Open an issue**: https://github.com/manny-est/flowpilot/issues\n\n" +
    ...
    // /feedback: feature requests, complaints, or anything that isn't a
    // straightforward bug — same GitHub issues page as /bug, distinguished
    // by convention (put "feedback" in the title) rather than a separate
    // tracker, plus a private route via the Node-RED forum.
    var FEEDBACK_TEXT = "## Feedback & feature requests\n\n" +
        "Got an idea, or something that doesn't feel right? I'd love to hear " +
        "about it.\n\n" +
        "- **Open an issue** with \"feedback\" in the title: " +
        "https://github.com/manny-est/flowpilot/issues\n" +
        "- **Or DM me directly** on the Node-RED forum: " +
        "https://discourse.nodered.org/u?name=manny-est\n\n" +
        "Found an actual bug instead? Try `/bug`.";
    ...
    var BETA_TEXT = "## Beta channel\n\n" +
        ...
        "**Found a bug?** Type `/bug`, or go straight to " +
    ...
    var SLASH_COMMANDS = [
        ...
        { cmd: "/feedback", desc: "Feature requests / general feedback" },
    ...
    function initMainWindow() {
            ...
            var content = $(
                ...
                '      <input id="fp-provider-type" type="hidden" value="openai-compatible">' +
                '      <label>Provider</label>' +
                '      <select id="fp-provider-preset"></select>' +
                '      <div id="fp-preset-docs-hint" class="fp-consent-hint fp-hidden"></div>' +
                '      <div id="fp-api-key-row">' +
                '        <label>API Key</label>' +
                ...
                '      <label>Assistant capability</label>' +
                '      <div id="fp-tier-status" class="fp-tier-status">Not checked yet — run Pre-flight check below.</div>' +
                '      <div class="fp-consent-hint">FlowPilot checks whether this model can reliably take ' +
                '        actions on your flows, and adjusts itself automatically — full control for capable ' +
                '        cloud models, a safer ask-then-act mode for anything local or that can\'t call tools ' +
                '        at all. Nothing to configure here normally; an override is available under Advanced ' +
                '        for anyone who wants to force a specific mode.</div>' +
                '      <div class="fp-settings-actions">' +
                ...
                '      <details class="fp-provider-advanced">' +
                ...
                '      <div id="fp-resolved-endpoint-hint" class="fp-consent-hint fp-hidden"></div>' +
                '      <div class="fp-consent-hint">Pre-filled from the Provider preset above — override only if ' +
                '        this instance runs somewhere non-standard. A URL ending in ' +
                '        <code>/chat/completions</code> is called exactly as typed; ending in <code>/v1</code> ' +
                '        gets that one path appended; anything else gets <code>/v1/chat/completions</code> ' +
                '        appended, same as before.</div>' +
                '      <div id="fp-openrouter-advanced" class="fp-hidden">' +
                '        <label><input id="fp-openrouter-zdr" type="checkbox"> Enforce ZDR (zero data retention) on requests</label>' +
                '        <div class="fp-consent-hint">Sends <code>provider:{zdr:true}</code> with every request. ' +
                '          This can only ENFORCE zero-data-retention routing per request — it cannot disable ' +
                '          account-level ZDR enforcement if your OpenRouter account requires it already.</div>' +
                '        <label><input id="fp-openrouter-attribution" type="checkbox"> Send attribution headers</label>' +
                '        <div class="fp-consent-hint">Identifies FlowPilot to OpenRouter via ' +
                '          <code>HTTP-Referer</code>/<code>X-Title</code> — app identity, not user data. Off by default.</div>' +
                '      </div>' +
                ...
                '      <label>Assistant capability override</label>' +
                '      <select id="fp-routing-tier-override">' +
                '        <option value="auto">Auto (recommended)</option>' +
                '        <option value="A">Full — plans and acts with tools</option>' +
                '        <option value="B">Limited — simplified tools only</option>' +
                '        <option value="C">None — classify only, no tool calls</option>' +
                '      </select>' +
                '      <div id="fp-tier-detection-reason" class="fp-consent-hint"></div>' +
                '      <div class="fp-consent-hint">Auto lets FlowPilot decide based on the Pre-flight ' +
                '        check\'s own tool-calling test and where this provider is hosted. Override this if ' +
                '        the detected reason above looks wrong for your setup (e.g. a LAN gateway in front ' +
                '        of a cloud model, or a local model reachable only through a public hostname or ' +
                '        tunnel) — or if you simply know this model\'s real capability better than the ' +
                '        automatic check.</div>' +
                '      </details>' +
                '      </details>' +
            ...
            content.find("#fp-provider-preset").on("change", function () {
                applyPresetToForm($(this).val(), true);
            });
            ...
            content.find("#fp-base-url").on("input", updateResolvedEndpointHint);
