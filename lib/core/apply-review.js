    ...
    function applyInsertions(newNodes, newWires, contextNodeIds) {
        ...
        Object.keys(componentAnchors).forEach(function (c) {
            ...
            // Walk a 2-wide grid outward from the natural origin until we
            // find a slot nothing else already occupies. Narrower than the
            // original 4-wide layout, and the y-step (90px) is larger than
            // the x-step (50px), so components separate vertically/
            // diagonally after just one collision instead of staying on the
            // same horizontal band for 3 more slots first — wires fan out
            // and stay visually distinguishable much sooner.
            var slot = 0;
            var x = baseX, y = baseY;
            while (collidesWithOccupied(x, y) && slot < 200) {
                slot++;
                x = baseX + (slot % 2) * 50;
                y = baseY + Math.floor(slot / 2) * 90;
            }
            if (slot > 0) {
