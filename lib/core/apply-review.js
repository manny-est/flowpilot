    var DIFF_SKIP = {
        ...
        nodes: 1,
        // Node appearance/documentation fields. These have normal default
        // values (empty strings, empty arrays) in freshly-imported nodes
        // but models hallucinate sentinel placeholders for them during
        // build-loop reviews, causing every node to appear blocked with
        // no actionable remediation. These are only configurable via the
        // node editor's Appearance tab — never through a Modify patch.
    ...
    function addModifyReview(modifiedFlow, newNodes, newWires, removeNodes, applyCallback, buildFixInfo, newGroups) {
        ...
        if (redactionOnlyDiffs.length > 0) {
            ...
            $("<div>").addClass("fp-review-count").css("margin-top", redSectionTop)
                .text("Not applied — FlowPilot can’t change these directly:")
                .appendTo($summaryPanel);
            redactionOnlyDiffs.forEach(function (d) {
                ...
                $("<div>").addClass("fp-diff-warn")
                    .text("Can’t change “" + fieldList + "” — this field is stored as a redacted placeholder (encrypted credential or config node value). Open the node editor and change it directly.")
                    .appendTo($section);
            });
        }
        ...
        if (!hasAnyChanges) {
            if (totalRedactionSkips > 0) {
                $("<div>").addClass("fp-warning")
                    .text("None of the proposed changes could be applied — all target fields that FlowPilot can only see as placeholders (encrypted credentials or config node values). See the field names above. Edit those directly in the node editor, then re-run the request for any remaining changes.")
                    .appendTo($actions);
            } else {
