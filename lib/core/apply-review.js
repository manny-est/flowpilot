    ...
    function addGeneratedReview(flow, onImported, buildGoal) {
        ...
        if (v.brokenWires.length) {
            ...
            if (onImported) {
                ...
                // plain escape hatch. Power users who just want to drop nodes without
                // the verification loop can use "Just add to canvas".
                ...
                $("<span>").addClass("fp-review-hint").text("Skips the verification loop.").appendTo($actions);
            } else {
                var $addBtn = $("<button>")
                    .addClass("red-ui-button red-ui-button-primary")
                    .attr("type", "button")
