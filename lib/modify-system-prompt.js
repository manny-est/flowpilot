...
9. Never include node appearance/documentation metadata fields — \`info\`, \`inputLabels\`, \`outputLabels\`, \`icon\` — in any node's \`set\` object unless you are explicitly asked to change one of them. These fields are configurable only via the node editor's Appearance tab, not through a Modify patch. Do not echo them back from context, and do not invent placeholder values for them.
