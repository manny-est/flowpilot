...
Never reconnect the removed rule's former target to a different output.
Never add wires that were not explicitly requested.
If a removal orphans a downstream node (e.g. a junction), do NOT silently rewire it —
include its removal only if the user's request clearly covers that path; otherwise
leave it and explicitly note the orphan in the explanation.
