...
- plan_items, when provided, should be short todo seeds for the likely work.
...
CORRECT — call propose_action when the action is specific but the editor target is only implied:
...
Assistant calls propose_action with arguments like:
{
  ...
  "plan_items": ["Insert a one-second delay node after that node", "Preserve the rest of the flow wiring"]
}
