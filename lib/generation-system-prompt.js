...
- It asks for durable documentation of the existing flow/selection — a read-me, walkthrough, explanation to keep on the canvas, or similar artifact — with nothing new to build. That's a Document request.
- It's a question, troubleshooting request, review, or conversational remark where the answer itself is the goal and no new nodes should be created at all. That's Chat, even if the topic is the existing flow/selection.
- If the user wants an explanation right now, prefer Chat. Use Document only when they are explicitly asking for documentation output they can keep or drop onto the canvas.
...
- "mode": "modify" if they want an actual change to existing nodes; "document" only for durable documentation output; "chat" for questions, troubleshooting discussion, or explanatory conversation with no documentation artifact needed.
